...
void AlgorithmB(network_type *network, algorithmBParameters_type *parameters) {
...
#if PAR
    declareMatrix(long, LPtree_par, NUM_THREADS, network->numNodes);
    declareMatrix(long, SPtree_par, NUM_THREADS, network->numNodes);
	declareMatrix(double, LPcosts_par, NUM_THREADS, network->numNodes);
    declareMatrix(double, SPcosts_par, NUM_THREADS, network->numNodes);
#endif
   ...
   /* Iterate */
	...
	// /* Update bushes */
	// 	for (origin = network->numZones / 2; origin < network->numZones; origin++) {
	...
	// /* Update bushes */
	// 	for (origin = 0; origin < network->numZones / 2; origin++) {
	// 		updateBushB(origin, network, inBush[origin], bushFlows[origin], SPcosts, LPcosts, bushOrder[origin], SPtree, LPtree, parameters);
	// 		//updateFlowsB(origin, network, inBush[origin], bushOrder[origin], bushFlows[origin], parameters);
	// 	}
